...
$$
\begin{aligned}
\pi(\boldsymbol{\Theta} |\boldsymbol{Y}) & =\pi(\textbf{B}^\top, \boldsymbol{\mu}^\top, \sigma^2, \boldsymbol{\Sigma}\mid Y) \propto \underbrace{\prod\limits_{i=1}^{n} f(\boldsymbol{Y}_i\mid\boldsymbol{\beta}_i,  \sigma^2 )}_{\text{likelihood of } \boldsymbol{Y}}\underbrace{\prod\limits_{i=1}^{n}\pi(\boldsymbol{\beta}_i \mid \boldsymbol{\mu},  \boldsymbol{\Sigma})}_{\text{distribution of }\textbf{B}}\underbrace{P(\sigma^2)P(\boldsymbol{\mu})P(\boldsymbol{\Sigma}^{-1})}_{\text{priors}}\\
