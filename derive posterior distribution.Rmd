...
For $i^{th}$ hurricane, there may be $m_i$ times of record (excluding the first observation), let
$$\boldsymbol{Y}_i = 
\begin{pmatrix}
Y_i(t_0+6)\\
Y_i(t_1+6)\\
...
Therefore, since $Y_i(t)$'s are independent across $t$
...
So, by using Bayesian rule, we have the following posterior distribution:
...
To apply MCMC, we need to find conditional posterior distribution of each parameter, then we can implement Gibbs sampling on these conditional posterior distributions.



1. Finding the posterior distribution of $\textbf{B}$ 

Since finding the posterior distribution of $\textbf{B}$ is the same to find the posterior distribution of $\boldsymbol{\beta_i}$, we try to derive the conditional distribution $\pi(\boldsymbol{\beta_i} | .)$
...
& = \exp\{-\frac{1}{2}\Big(\boldsymbol{\beta}_i\boldsymbol{V}\boldsymbol{\beta}_i^\top-\boldsymbol{M}\boldsymbol{\beta}_i^\top-\boldsymbol{\beta}_i\boldsymbol{M}_i^\top+\boldsymbol{R}\Big)\}
\end{aligned}$$
...
2. Finding the posterior distribution of $\pi(\sigma^2| .)$
...
3. Finding the posterior distribution of $\pi(\boldsymbol{\Sigma} | .)$
...
4. Finding the posterior distribution of $\pi(\boldsymbol{\mu} | .)$
