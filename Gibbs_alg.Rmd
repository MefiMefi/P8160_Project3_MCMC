...
MCMC.Gibbs <- function(dt, init.muvec, init.sigma2, init.Sigma, max.iter = 1e4){
    ...
    cur = list(B = init.Bstuff$B, sigma2 = init.sigma2, muvec = init.muvec, Sigma = init.Sigma ,RSS = init.Bstuff$RSS)
...
```{r test_mcmc}
# run 10 cycles
#test.MCMC.res = MCMC.Gibbs(dt_mtx_train, init.muvec = c(1,2,3,4,5), init.sigma2 = 1, init.Sigma = diag(0.3, 5, 5), max.iter = 100)
```
...
MCMC.res.2 = MCMC.Gibbs(dt_train, init.muvec = c(1,2,3,4,5), init.sigma2 = 5, init.Sigma = diag(c(1,2,3,4,5)))
